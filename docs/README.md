...
- [x] 구입 가격에 숫자가 아닌 다른 문자(공백 포함)가 포함되어 있으면 예외가 발생한다.
- [x] 구입 가격에 0으로 시작하는 숫자가 들어가면 예외가 발생한다.
- [x] 구입 가격이 1000(원)으로 나누어 떨어지지 않으면 예외가 발생한다.
- [x] 구입 가격이 최소 금액인 1000(원)보다 적으면 예외가 발생한다.
...
- [x] 로또 번호와 당첨 번호를 비교했을 때 3개가 일치하면 5를 반환한다.
- [x] 로또 번호와 당첨 번호를 비교했을 때 4개가 일치하면 4를 반환한다.
- [x] 로또 번호와 당첨 번호를 비교했을 때 5개가 일치하면 3을 반환한다.
- [x] 로또 번호와 당첨 번호를 비교했을 때 5개가 일치하고 보너스 번호와 일치하면 2를 반환한다.
- [x] 로또 번호와 당첨 번호를 비교했을 때 6개가 전부 일치하면 1을 반환한다.
- [x] 전체 구매한 로또 번호의 결과가 [1등, 2등, 3등, 4등, 5등] 형식으로 반환된다.
- [x] 당첨 결과를 토대로 수익을 반환 받는다.
