...
- [ ] 1000원 단위의 구입 금액을 사용자로부터 입력받는다.
  - 입력된 구입 금액이 `⛔ 에러 메시지 기능` - `⚠️ 구입 금액 입력 상황` 에 해당하는지 검사한다.
- [ ] 당첨 번호를 입력 받는다. 이때, 번호는 쉼표(,)를 기준으로 구분한다.
  - 입력된 당첨 번호가 `⛔ 에러 메시지 기능` - `⚠️ 당첨 번호 입력 상황` 에 해당하는지 검사한다.
- [ ] 1~45 이내의 보너스 번호를 입력 받는다.
  - 입력된 당첨 번호가 `⛔ 에러 메시지 기능` - `⚠️ 보너스 번호 입력 상황` 에 해당하는지 검사한다.<br>
    <br>

### 2️⃣ 결과 출력 기능

- [ ] 로또 구입 금액을 입력 받기 위해 `구입 금액을 입력해 주세요.` 을 출력한 뒤 구입 금액을 입력 받는다. 이후 개행한다.
- [ ] 구매한 로또의 총 개수를 `N개를 구매했습니다.` 와 같이 출력한다. (로또는 1000원당 1개다.)
- [ ] 구매한 로또 개수만큼 `[8, 21, 23, 41, 42, 43]` 의 형태로 출력한다. 이후, 개행한다.
- [ ] 출력할 로또 번호의 정렬은 `5️⃣ 로또 번호 정렬 기능` 을 활용한다.
- [ ] 당첨 번호를 입력 받기 위해 `당첨 번호를 입력해 주세요.` 를 출력한 뒤 당첨 번호를 입력 받는다. 이후, 개행한다.
- [ ] 보너스 번호를 입력 받기 위해 `보너스 번호를 입력해 주세요` 를 출력한 뒤 보너스 번호를 입력 받는다. 이후, 개행한다.
- [ ] 당첨 통계를 알려주기 위해 `당첨 통계` 를 출력한다.
- [ ] 구분을 위해 `---` 를 출력한다.
- [ ] 당첨 통계에 해당하는 정보를 각 라인에 이어서 출력한다. 당첨 통계 구분은 다음과 같다. (1개, 2개 일치는 출력하지 않는다.)
  - `3개 일치 (5,000원) - X개`
  - `4개 일치 (50,000원) - Y개`
  - `5개 일치 (1,500,000원) - Z개`
  - `5개 일치, 보너스 볼 일치 (30,000,000원) - N개`
  - `6개 일치 (2,000,000,000원) - M개`
- [ ] 총 수익률을 계산하여 `총 수익률은 xx.x%입니다.` 와 같이 출력한다.<br>
      <br>

### 3️⃣ 금액에 맞는 로또 개수 발행 기능

- [ ] 1000원 단위로 입력된 금액에 맞게 로또 개수를 계산한다.<br>
      <br>

### 4️⃣ 랜덤 번호 생성 기능

- [ ] 입력된 금액 에 맞는 개수 만큼 로또를 생성한다.
...
- [ ] 입력된 당첨번호와 생성된 로또 번호와의 일치 번호 개수를 계산한다.
- [ ] `0개 일치`, `1개 일치`, `2개 일치` 인 로또 번호는 값을 저장하지 않는다.<br>
      <br>

### 7️⃣ 모든 로또의 일치 정보를 계산 하는 기능

- [ ] 각 로또들이 몇 개의 일치 값을 가지고 있는지 확인한 뒤, 그 결과를 카운트한다.
- [ ] `3개 일치`, `4개 일치`, `5개 일치`, `5개 일치, 보너스 볼 일치`, `6개 일치` 에 대한 개수를 모두 카운트 한다.<br>
      <br>

### 8️⃣ 수익률 계산 기능

- [ ] 구매한 로또 금액 대비 당첨 금액 수익률을 계산한다.
- [ ] 계산 방법은 `당첨 금액 / 구매한 로또 금액` 이다.
- [ ] 당첨 금액은 다음과 같다.
  - 3개 일치 => `5,000원`
  - 4개 일치 => `50,000원)`
  - 5개 일치 => `1,500,000원`
...
- [ ] 구매금액이 1000원 단위가 아니라면 `[ERROR] 구매 금액은 1000원 단위로 입력하셔야 합니다.` 라는 에러 메시지를 출력 후 종료한다.
- [ ] 공백을 포함한 숫자 이외의 모든 값이 입력으로 들어온다면 모두 예외 상황이다. `[ERROR] 구매 금액은 1000원 단위로 입력하셔야 합니다.` 라는 에러 메시지를 출력 후 종료한다.<br><br>

### ⚠️ 당첨 번호 입력 상황

- [ ] 5개의 `,` 로 구분된 1~45 이내의 6개의 숫자가 입력되어야 한다. 공백을 포함한 이외의 모든 입력들은 `[ERROR] 올바른 당첨 번호를 입력해주세요.` 라는 에러 메시지를 출력 후 종료한다.
- [ ] 입력 숫자는 1~45 이내여야 한다. 이외의 숫자 및 문자들이 입력될 시 `[ERROR] 1~45 의 숫자만 입력할 수 있습니다.` 라는 에러 메시지를 출력 후 종료한다.<br><br>

### ⚠️ 보너스 번호 입력 상황

- [ ] 1~45 이내의 1개의 숫자가 입력되어야한다. 공백을 포함한 이외의 모든 입력들은 `[ERROR] 올바른 보너스 번호를 입력해주세요.` 라는 에러 메시지를 출력 후 종료한다.
- [ ] 입력 숫자는 1~45 이내여야 한다. 이외의 숫자 및 문자들이 입력될 시 `[ERROR] 1~45 의 숫자만 입력할 수 있습니다.` 라는 에러 메시지를 출력 후 종료한다.
- [ ] 입력된 보너스 번호가 당첨 번호와 중복될 시에 `[ERROR] 보너스 번호는 당첨번호와 중복되면 안됩니다.` 라는 에러 메시지를 출력 후 종료한다.
      <br><br>
