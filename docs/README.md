...
- [x] 구매금액이 1000원 단위가 아니라면 `[ERROR] 구매 금액은 1000원 단위로 입력하셔야 합니다.` 라는 에러 메시지를 출력 후 종료한다.
- [x] 공백을 포함한 숫자 이외의 모든 값이 입력으로 들어온다면 모두 예외 상황이다. `[ERROR] 구매 금액은 1000원 단위로 입력하셔야 합니다.` 라는 에러 메시지를 출력 후 종료한다.<br><br>

### ⚠️ 당첨 번호 입력 상황

- [x] 5개의 `,` 로 구분된 1~45 이내의 6개의 숫자가 입력되어야 한다. 공백을 포함한 이외의 모든 입력들은 `[ERROR] 올바른 당첨 번호를 입력해주세요.` 라는 에러 메시지를 출력 후 종료한다.
- [x] 입력 숫자는 1~45 이내여야 한다. 이외의 숫자 및 문자들이 입력될 시 `[ERROR] 1~45 의 숫자만 입력할 수 있습니다.` 라는 에러 메시지를 출력 후 종료한다.<br><br>
...
- [x] 입력된 보너스 번호가 당첨 번호와 중복될 시에 `[ERROR] 보너스 번호는 당첨번호와 중복되면 안됩니다.` 라는 에러 메시지를 출력 후 종료한다.
      <br><br>
