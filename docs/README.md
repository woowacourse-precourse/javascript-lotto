...
- [x] 당첨번호 입력 `Lotto`
  ...
  - [x] 각 번호의 구분을 `,`로 한다.
  - [x] 번호가 중복되면 안된다.
- ~~ 보너스번호 입력 `LottoBonus` ~~ (리펙토링으로 Lotto와 통합)
  - [x] 번호는 1~45의 숫자다.
...
- [x] `LottoBonus` 클래스 `Lotto`클래스와 통합
- [x] `LottoGame` 클래스 생성자 제거 후 프라이빗 필드 추가
  - [ ] 이에 따른 `LottoGame`에 종속된 클래스 변경
...
- 당첨번호와 구매한 로또를 비교해 당첨 데이터를 구한다. `calculateResult`
  - 구매한 로또를 한개씩 확인한다.
    ...
    - 중복되는 갯수에 따른 수입을 추가한다.
      - ex) 3개가 중복되면 `result.profit`을 5000 증가시킨다.
...
- 구입금액과 당첨금액을 활용해 수익률을 구한다. `calculateRate`
  - 수익률은 둘째자리에서 반올림한다.
  - 수익률은 다음 공식이 적용된다.
    - `( 얻은 금액 / 소비 금액 * 100 ) %`
