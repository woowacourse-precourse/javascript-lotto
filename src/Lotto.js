...
class Lotto {
  ...
  validate(numbers) {
    ...
    if (numbers.filter((number) => 1 <= number && number <= 45).length != 6) throw new Error('[ERROR] 로또 번호는 1에서 45 사이의 숫자입니다.');

    if (!checkOverlap()) throw new Error('[ERROR] 로또 번호에 중복된 숫자가 있습니다.');
  }

  result(playerNumber, bonusNumber) {
    if (this.#numbers.includes(bonusNumber)) throw new Error(`${MESSAGE.ERROR} 보너스 번호와 당첨 번호가 중복됩니다.`);
