//예외 사항들
export const exceptLottoNumbers = (lottoNumber) => {
    const NUM_REG = /[^0-9]/g;
...
export const exceptLottoPrice = (lottoPrice) => {
    if (lottoPrice % 1000 !== 0) {
        throw new Error('[ERROR] 로또 구입 금액은 1000원 단위여야 합니다.');
    }
};

export const exceptBonusNumber = (winningNumbers = [], bonusNumber) => {
    winningNumbers.map((winningNumber) => {
        if (winningNumber === bonusNumber) {
            throw new Error('[ERROR] 보너스 번호는 로또 당첨번호와 동일해서는 안됩니다.');
        }
    });
};
