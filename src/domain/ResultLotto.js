...
class ResultLotto{
    ...
    printResult(){
        this.winCounts.push(this.resultLottos.reduce((count, element) => count + ('3match' === element), 0))
        MissionUtils.Console.print(`3개 일치 (5,000원) - ${this.winCounts[0]}개`);
        this.winCounts.push(this.resultLottos.reduce((count, element) => count + ('4match' === element), 0))
        MissionUtils.Console.print(`4개 일치 (50,000원) - ${this.winCounts[1]}개`);
        this.winCounts.push(this.resultLottos.reduce((count, element) => count + ('5match' === element), 0))
        MissionUtils.Console.print(`5개 일치 (1,500,000원) - ${this.winCounts[2]}개`);
        this.winCounts.push(this.resultLottos.reduce((count, element) => count + ('bonusmatch' === element), 0))
        MissionUtils.Console.print(`5개 일치, 보너스 볼 일치 (30,000,000원) - ${this.winCounts[3]}개`);
        this.winCounts.push(this.resultLottos.reduce((count, element) => count + ('6match' === element), 0))
