...
class Calculator {
  createLottoNumber (count) {
    ...
    const arr = Object.keys(lottoNumber).map(item => lottoNumber[item]);
  ...
  winningScore (result, arr, number) {
    const count = new Array(result.length).fill(0);
    for (let index = 0; index < result.length; index++) {
      if (result[index].length === 6) {
        count[7] += 1;
      } else if (result[index].length === 5 && arr[index].includes(number)) {
        count[6] += 1;
