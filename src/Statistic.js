...
const Statistic = class {
  ...
  getRankResult({ lottos, winNumber, bonusNumber }) {
    ...
    lottos.forEach((lotto) => {
      ...
      const isRankTwo =
        this.hasIncludesBonusNumber({ lotto, bonusNumber }) &&
        matchCount === LOTTO_RANK.CASE.RANK_TWO;
      const rankResult = isRankTwo
        ? LOTTO_RANK.NAME.RANK_TWO
        : this.calculateRank(matchCount);
  ...
  getYield({ rank, lottos }) {
    ...
    const winAmounts = [];
    rank.forEach((count, rank) =>
      winAmounts.push(this.calculateYield(rank) * count)
    );
    return (
      (winAmounts.reduce((acc, cur) => acc + cur, 0) / buyAmount) *
      100
    ).toFixed(1);
  }

  getRankTwoMatchingCount({ lottos, winNumber }) {
    return lottos.filter(
      (lotto) =>
        this.getMatchingCount({ lotto, winNumber }) === LOTTO_RANK.CASE.RANK_TWO
    ).length;
  }

  getMatchingCount({ lotto, winNumber }) {
    return lotto.reduce(
      (count, number) => (winNumber.includes(number) ? count + 1 : count),
      0
    );
  }
