...
const validateWinningNumber=(winningNumber)=>{
  if (!winningRepeatedCheck(winningNumber)) throw new Error(REPEATED_WINNING_ERROR)

  if (!winningRangeCheck(winningNumber)) throw new Error(RANGE_WINNING_ERROR)

  if (!winningIncludeString(winningNumber)) throw new Error(STRING_WINNING_ERROR)

  if (!winningLengthCheck(winningNumber)) throw new Error(LENGTH_WINNING_ERROR)

  if (!winningIncludeFloatNumber(winningNumber)) throw new Error(FLOAT_WINNING_ERROR)
}

const validateBonus=(numbers,bonus)=>{
  if (!winningIncludeBonusNumber(numbers,bonus)) throw new Error(BONUS_WINNING_ERROR)
}
