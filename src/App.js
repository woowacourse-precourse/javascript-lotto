...
class App {
    ...
    MissionUtils.Console.readLine(
      ...
      (moneyToBuyLottos) => {
        ...
        if (!regex.test(moneyToBuyLottos))
          throw new Error(
            "[ERROR] 구입 금액은 1000원 단위의 금액을 입력해주세요"
          );
  ...
  createIssuedLottoList() {
    for (let i = 0; i < this.#numberOfLottos; i++) {
      const tempRandomNumbers = MissionUtils.Random.pickUniqueNumbersInRange(
        1,
        45,
        6
      );
    ...
    MissionUtils.Console.readLine(
      ...
      (inputNumbers) => {
        ...
        if (new Set(this.#lottoWinningNumbers).size != 6)
          throw new Error(
            "[ERROR] 로또 번호는 서로 다른 6개의 숫자여야 합니다."
          );
      }
    );
  }
  ...
  printLottoWinningResult() {
    ...
    for (let i = 0; i < this.#numberOfLottos; i++) {
      ...
      if (prize <= 5) countOfLottoWinning[prize]++;
    }
    MissionUtils.Console.print(
      `3개 일치 (${PRIZE_MONEY[5].toLocaleString("ko-KR")}원) - ${
        countOfLottoWinning[5]
      }개`
    );
    MissionUtils.Console.print(
      `4개 일치 (${PRIZE_MONEY[4].toLocaleString("ko-KR")}원) - ${
        countOfLottoWinning[4]
      }개`
    );
    MissionUtils.Console.print(
      `5개 일치 (${PRIZE_MONEY[3].toLocaleString("ko-KR")}원) - ${
        countOfLottoWinning[3]
      }개`
    );
    MissionUtils.Console.print(
      `5개 일치, 보너스 볼 일치 (${PRIZE_MONEY[2].toLocaleString(
        "ko-KR"
      )}원) - ${countOfLottoWinning[2]}개`
    );
    MissionUtils.Console.print(
      `6개 일치 (${PRIZE_MONEY[1].toLocaleString("ko-KR")}원) - ${
        countOfLottoWinning[1]
      }개`
    );
    for (let i = 1; i < 6; i++)
  ...
  printRateOfReturn() {
    const rateOfReturn =
      Math.round(this.#totalLottosPrizeMoney / this.#numberOfLottos) / 10;
    MissionUtils.Console.print(`총 수익률은 ${rateOfReturn}%입니다.`);
  }
  getPrize(numbers) {
    const countOfMatchNumber = numbers.getCountOfMatchNumber(
    ...
    if (countOfMatchNumber == 5) {
      if (numbers.getCountOfMatchNumber(this.#lottoBonusNumber) == 1) return 2;
      return 3;
    }
