const isValidateNumber = (number) => {
  ...
  if (!number.match(numberRegex)) throw new Error('[ERROR] 숫자가 아닌 값은 입력할 수 없습니다.');
};

const isAmountUnitOf1000 = (number) => {
  if (number % 1000 !== 0) throw new Error('[ERROR] 구입금액은 1000 단위 입니다.');
};

const isZeroNumber = (number) => {
  if (number === '0') throw new Error('[ERROR] 최소금액은 1000원입니다.');
};

const isLottoRange = (number) => {
  if (number < 1 || number > 45) throw new Error('[ERROR] 로또번호는 1부터 45까지입니다.');
};
