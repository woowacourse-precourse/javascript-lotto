...
const LottoNumberGenerator = require('../src/components/LottoNumberGenerator');
...
describe('로또 번호 추첨기 클래스 테스트', () => {
  ...
  test('당첨 번호에 중복된 숫자가 있으면 예외가 발생한다.', () => {
    const errorMsg = makeErrorMsg(ERROR_MESSAGE.WINNER_DUPLICATION);
  ...
  test('보너스 번호에 중복된 숫자가 있으면 예외가 발생한다.', () => {
    const errorMsg = makeErrorMsg(ERROR_MESSAGE.BONUS_DUPLICATION);
    const answers = ['1,2,3,4,5,6', '6'];
