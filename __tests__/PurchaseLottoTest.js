...
describe('로또 구입 테스트', () => {
  test('금액에 맞개 로또를 구매하는지 테스트', () => {
    ...
    expect(PURCHASE_LOTTO.purchasedLottoNumbers.length).toBe(5);
  });

  test('구매한 로또 번호가 6개인지 테스트', () => {
    const PURCHASE_LOTTO = new PurchaseLotto('5000');
    PURCHASE_LOTTO.purchasedLottoNumbers.forEach((lotto) => {
  ...
  test('구매한 로또 번호가 1 ~ 45 사이인지 테스트', () => {
    ...
    const NUMBERS = PURCHASE_LOTTO.purchasedLottoNumbers[0];
    NUMBERS.forEach((num) => {
