...
describe('Validator 클래스 테스트', () => {
  test('Number가 아닌 타입에 대해서 Error를 발생하는지 테스트', () => {
    expect(() => {
      Validator.validateTypeNumber(Number('1000j'));
    }).toThrow('[ERROR]');
  });

  test('1000보다 작은 자리수를 가지고 있는지 validation이 가능한가', () => {
    expect(() => {
      Validator.validateUnitRemainder(1030);
    }).toThrow('[ERROR');
  });
  ...
  test('array의 길이에 따른 validation이 가능한가', () => {
    ...
    expect(() => {
      Validator.validateLength(example);
    }).toThrow('[ERROR]');
  });
  ...
  test('array에 곂치는 수가 들어있는지 validation이 가능한가', () => {
    ...
    expect(() => {
      Validator.validateOverlapNumbers(example);
    }).toThrow('[ERROR]');
  });

  test('입력값이 제한 범위에 알맞게 입력되었는지 validation할 수 있는가', () => {
    expect(() => {
      Validator.validateRange(48);
    }).toThrow('[ERROR]');
  });
});
