...
describe("상금 확인 테스트", () => {
  ...
  test("4개의 번호가 일치-4등", () => {
    ...
    expect(AWARD.haveAward()).toBe(50000);
  });
  test("5개의 번호가 일치-3등", () => {
    const AWARD = new Award(correctArr3, bonusBoolean);
    expect(AWARD.haveAward()).toBe(1500000);
  });
  test("5개의 번호가 일치 + 보너스 번호가 일치-2등", () => {
    bonusBoolean = true;
    const AWARD = new Award(correctArr4, bonusBoolean);
    expect(AWARD.haveAward()).toBe(30000000);
  });
  test("6개의 번호가 일치-1등", () => {
    const AWARD = new Award(correctArr5, bonusBoolean);
    expect(AWARD.haveAward()).toBe(2000000000);
  });
});
