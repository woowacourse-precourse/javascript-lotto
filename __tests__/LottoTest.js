...
describe('로또 보너스 번호 검사', () => {
  test('정상', () => {
    const winningNumbers = [16, 26, 29, 31, 36, 8];
    const bonusNumber = 11;

    expect(() => Lotto.validateBonusNumber(bonusNumber, winningNumbers)).not.toThrow();
  });
  ...
  test('예외: 보너스 번호가 숫자가 아닐 경우', () => {
    ...
    expect(() => Lotto.validateBonusNumber(bonusNumber, winningNumbers)).toThrow('[ERROR]');
  });
