...
describe("로또 클래스의 getResult 함수 테스트", () => {
  test.each([
    [[42, 12, 11, 2, 9, 23], [42, 12, 11, 2, 9, 23], 1],
    [[18, 31, 36, 2, 40, 12], [2, 12, 18, 31, 36, 40], 9],
    [[19, 5, 8, 6, 7, 12], [5, 8, 6, 7, 12, 19], 45],
  ])(
    "로또 번호가 6개가 일치하면 1을 반환한다. 나의번호: %p, getResult(%p, %d) => 1",
    (numbers, winningNumbers, bonusNumber) => {
      const lotto = new Lotto(numbers);

      expect(lotto.getResult(winningNumbers, bonusNumber)).toEqual(1);
    }
  );
  ...
  test.each([
    ...
    "로또 번호가 5개가 일치하고 다른 하나가 보너스 번호라면 2를 반환한다. 나의번호: %p, getResult(%p, %d) => 2",
    (numbers, winningNumbers, bonusNumber) => {
      const lotto = new Lotto(numbers);

      expect(lotto.getResult(winningNumbers, bonusNumber)).toEqual(2);
    }
  );
  ...
  test.each([
    ...
    "로또 번호가 5개가 일치하면 3을 반환한다. 나의번호: %p, getResult(%p, %d) => 3",
    (numbers, winningNumbers, bonusNumber) => {
      const lotto = new Lotto(numbers);

      expect(lotto.getResult(winningNumbers, bonusNumber)).toEqual(3);
    }
  );
  ...
  test.each([
    ...
    "로또 번호가 4개가 일치하면 4을 반환한다. 나의번호: %p, getResult(%p, %d) => 4",
    (numbers, winningNumbers, bonusNumber) => {
      const lotto = new Lotto(numbers);

      expect(lotto.getResult(winningNumbers, bonusNumber)).toEqual(4);
    }
  );
  ...
  test.each([
    ...
    "로또 번호가 3개가 일치하면 5을 반환한다. 나의번호: %p, getResult(%p, %d) => 5",
    (numbers, winningNumbers, bonusNumber) => {
      const lotto = new Lotto(numbers);

      expect(lotto.getResult(winningNumbers, bonusNumber)).toEqual(5);
    }
  );
  ...
  test.each([
    ...
    "로또 번호가 일치하지 않거나 2개 이하로 6또는 7, 8을 반환한다. 나의번호: %p, getResult(%p, %d) => %d",
    (numbers, winningNumbers, bonusNumber, rank) => {
      const lotto = new Lotto(numbers);

      expect(lotto.getResult(winningNumbers, bonusNumber)).toEqual(rank);
    }
  );
  ...
  test.each([
    ...
    "나의 번호: %p, getResult(%p, %d) => %d",
    (numbers, winningNumbers, bonusNumber, result) => {
      const lotto = new Lotto(numbers);

      expect(lotto.getResult(winningNumbers, bonusNumber)).toEqual(result);
    }
  );
});
